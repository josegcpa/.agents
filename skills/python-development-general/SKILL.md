...
```
def function(test: int, test_optional: int = 1):
    """
...
- When developing CLIs always use `argparse`. If you are developing a complex CLI which might require multiple endpoints (i.e. `cli-name train`, `cli-name process`) use subparsers
